...
- Fixed a `{% filter %}` block, and a filter on the output of a block `set`, a macro or `caller()`, getting `JinjaString.user` text already escaped: with `x = JinjaString.user('<b>')`, `{% filter upper %}{{ x }}{% endfilter %}` is now `&lt;B&gt;`, not `&LT;B&GT;`, and `{% filter length %}{{ x }}{% endfilter %}` is `3`, not `9`, as in llama.cpp. Input text is now escaped once, when the template outputs it. So `| safe` on such output, and `{% filter safe %}`, now turn escaping off for the input in it, as `{{ x | safe }}` does. A block whose output also contains `| safe` input text is still escaped before the filter runs, as `~` does.
